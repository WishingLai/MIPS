...
void readfile() {
	...
	fin1.open("memory02.txt", ios::in);		//讀取檔案
											//讀取 將程式碼放入a字串
	while (!fin1.eof())
	{
		getline(fin1, temp);
...
int main()
{
	...
	fin333.open("memory02.txt", ios::in);		//讀取檔案
	int coco = 0;
	while (!fin333.eof())
	{
	...
	fout.open("result02.txt", ios::out);	//清空,再輸入內容
										//讀取 將程式碼放入a字串
	int next_i = 0;
